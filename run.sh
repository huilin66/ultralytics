...
# E2.1 confirmation seed: run only baseline and the promising GIA@5+7
# candidate. Both runs use stage 1 only (100 epochs), seed=1 and w4=0.5.
python scripts/train_mdet_experiments.py gia-position \
  --label E2_1_GIA_v2_confirm \
  --data "$DATA" \
  --pretrain yolov10x.pt \
  --seed 1 \
  --stage1-only \
  ...
  --variant gia_v2_5_7=ultralytics/cfg/models/exp_ablation/yolov10x_GIA_v2_5_7.yaml \
  --project runs/experiments/E2_1_GIA_v2_confirm
