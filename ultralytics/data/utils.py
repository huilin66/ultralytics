...
def verify_image_label_mdet(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            nl = len(lb)
            if nl:
                attribute_len = int(lb[0, 1])
                if keypoint:
                    assert lb.shape[1] == 5+ nkpt * ndim +attribute_len+1, f"labels require {attribute_len+6+ nkpt * ndim} columns, {lb.shape[1]} columns detected : {os.path.basename(lb_file)}"
                    points = lb[:, 5+attribute_len+1:].reshape(-1, ndim)[:, :2]
                    mdet_attributes = lb[:, 2:attribute_len+2]
                else:
                    assert lb.shape[1] == 5+attribute_len+1, f"labels require {attribute_len+6} columns, {lb.shape[1]} columns detected : {os.path.basename(lb_file)}"
                    points = lb[:, attribute_len+2:]
                    mdet_attributes = lb[:, 2:attribute_len+2]
