...
def compute_ap(recall, precision):
    ...
    if method == "interp":
        ...
        # NumPy 2.x removed the long-standing ``np.trapz`` alias. Keep the
        # fallback for older NumPy versions used by some training machines.
        trapezoid = getattr(np, "trapezoid", None)
        ap = (trapezoid if trapezoid is not None else np.trapz)(np.interp(x, mrec, mpre), x)
    else:  # 'continuous'
