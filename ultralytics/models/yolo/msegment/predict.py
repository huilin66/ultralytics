    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        for i, pred in enumerate(p):
            ...
            if not len(pred):  # save empty boxes
                ...
                pred[:, :4] = ops.scale_boxes(img.shape[2:], pred[:, :4], orig_img.shape)
            results.append(MResults(orig_img, path=img_path, names=self.model.model.names, boxes=pred[:, :6], masks=masks,
                                    attributes=pred[:, 6:6+self.model.model.na], attribute_names=self.model.model.attribute_names))
        return results
