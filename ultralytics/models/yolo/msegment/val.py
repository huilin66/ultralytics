...
class MSegmentationValidator(MDetectionValidator):
    """
    ...
    def _prepare_pred(self, pred, pbatch, proto):
        ...
        pred_masks = self.process(proto, pred[:, 6+self.na:], pred[:, :4], shape=pbatch["imgsz"])
        ...
        if isinstance(preds[1], list):
            ...
            for pi in range(num_p):
                ...
                for si in range(size_b):
                    ...
                    # Evaluate
                    if nl:
                        stat["tp"], stat["ap"], stat["conf_mat"], stat["filter_small_gt"], stat["filter_small_pred"] = self._process_batch(predn, bbox, cls,
                                                                                       gt_attributes=mdet_attributes, pbatch=pbatch)
