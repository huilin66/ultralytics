...
def yolo_to_coco(yolo_dir, img_dir, output_file, categories, task='detect'):
    """
    将YOLO格式的标注转换为COCO格式。
...
def obj_coco_analysis(data_cfg, val_path, cfg_dir='../ultralytics/cfg', task=None):
    if task is None:
        if 'segment' in val_path:
            if 'msegment' in val_path:
                task='msegment'
            else:
                task='segment'
        else:
            if 'mdetect' in val_path:
                task='mdetect'
            else:
                task='detect'
    ...
    yolo_to_coco(data_val_label_dir, data_val_image_dir, data_val_coco_anno_path, categories, task=task)
...
if __name__ == '__main__':
    ...
    obj_coco_analysis('billboard_mdet5_10_c_0806m_det.yaml',
                 r'/localnvme/project/ultralytics/runs/mdetect/val851/predictions.json', task='detect')
    obj_coco_analysis('billboard_mdet5_10_c_0806m_det.yaml',
                 r'/localnvme/project/ultralytics/runs/mdetect/val852/predictions.json', task='detect')
